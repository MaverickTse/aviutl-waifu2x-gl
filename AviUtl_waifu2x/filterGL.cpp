...
void filterGLInit(uint32_t width, uint32_t height)
{
	...
	if (!loadShader("./shaders/convolve_vs.glsl", "./shaders/convolve_fs.glsl", &sConvoluve.program)) {
		MessageBox(NULL,"Error loading shader", "ERROR",MB_OK);
		return;
	}
	...
	if (!loadShader("./shaders/leaky_relu_vs.glsl", "./shaders/leaky_relu_fs.glsl", &sLeakyReLU.program)) {
		MessageBox(NULL, "shader compile error.", "ERROR", MB_OK);
		return;
	}
...
//		std::cout << log << std::endl;
		MessageBox(NULL, log, "VSH Shader Compile Error", MB_OK);
	    return false;
	}
...
//		std::cout << log << std::endl;
		MessageBox(NULL, log, "FSH Shader Compile Error", MB_OK);
	    return false;
	}
